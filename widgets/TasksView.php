 ...
 * @since 2.0
 */
class TasksView extends yii\grid\GridView
{
     ...
     */
    public function renderItems()                                             // ПЕРЕПИСАТЬ ВСЮ ФУНКЦИЮ К ЧЕРТУ!!!
    {
     ...
     * @return string the rendering result.
     */
    public function renderFilters()                                                    // ОТСЮДА НАЧИНАЕТСЯ ФИЛЬТР
    {
    ...
    {
        $models = array_values($this->dataProvider->getModels()); // возвращает массив моделей как раз
        $keys = $this->dataProvider->getKeys(); // id всех записей
        $rows = [];
